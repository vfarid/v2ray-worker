    ...
    <body dir="ltr">
      <div class="container border p-0">
        ...
        <div class="px-5 py-2 bg-light">
          ...
          <input id="sub-link" readonly value="https://${url2.hostname}/sub" class="p-1" style="width: calc(100% - 150px)">
        ...
        <div class="px-5 py-2 bg-light">
          ...
          <input id="clash-link" readonly value="https://${url2.hostname}/clash" class="p-1" style="width: calc(100% - 150px)">
...
async function PostPanel(request, env) {
  try {
    ...
    return Response.redirect(`https://${url2.hostname}?message=success`, 302);
  } catch (e) {
...
function MixConfig(cnf, url2, address, provider) {
  try {
    var conf = { ...cnf };
    ...
    conf.host = url2.hostname;
    conf.servername = url2.hostname;
    conf.server = address;
...
// src/collector.ts
async function GetConfigList(url2, env) {
  var maxConfigs = 200;
  var protocols = [];
  var providers = [];
  var alpnList = [];
  ...
  for (const i2 in acceptableConfigList) {
    ...
    acceptableConfigList[i2].mergedConfigs = el.configs.map((cnf) => MixConfig(cnf, url2, address, el.name)).filter((cnf) => cnf?.merged && cnf?.name);
  }
  var remaining = 0;
  for (var i = 0; i < 5; i++) {
  ...
  if (protocols.includes("vless")) {
    finalConfigList = finalConfigList.concat(
      await GetVlessConfigList(url2.hostname, cleanDomainIPs, env)
    );
  }
...
var worker_default = {
  async fetch(request, env) {
    const url2 = new URL(request.url);
    const path = url2.pathname.replace(/^\/|\/$/g, "");
    const outputType = path.toLowerCase();
    if (["sub", "clash"].includes(outputType)) {
      const configList = await GetConfigList(url2, env);
      if (outputType == "clash") {
